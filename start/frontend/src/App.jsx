...
export default function Dashboard() {
  const [ngrokUrl, setNgrokUrl] = useState("https://jimmie-shaggier-surgically.ngrok-free.dev");
...
const getData = async () => {
    try {
      const response = await axios.get(`${ngrokUrl}/data`, {
        headers: {
          "ngrok-skip-browser-warning": "69420", // This header bypasses the ngrok splash screen
        },
      });

      console.log("API Response:", response.data); // specific logging
      ...
      setReedState(reed_switch_state);
      
      setCurrentTime(moment().tz("Asia/Kolkata").format("hh:mm:ss A"));
    setTemperatureData((prevData) => [
        ...prevData,
        { x: new Date(), y: temperature },
      ]);
    setHumidityData((prevData) => [
        ...prevData,
        { x: new Date(), y: humidity },
      ]);
    setSoilMoistureData((prevData) => [
        ...prevData,
        { x: new Date(), y: soil_moisture },
      ]);
